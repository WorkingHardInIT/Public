#Typically, this is associated with the following events in the DNS log: 404,407 and 408
...
Function CheckDnsService() {
    ...
    #Create the "Veeam DNS Server Check" event source if it does not already exist
    ...
    If ( $DNSResults.Status -eq 'FAILED') {
        #Grab the logs containing the three suspect event ids
        $logs = get-eventlog 'DNS Server' -ComputerName VBR -source Microsoft-Windows-DNS-Server-Service -InstanceId "404", "407", "408" -After (Get-Date).AddMinutes(-15)
        If ( $Null -ne $logs) {
        ...
        Else {
            #The DNS test failed, but it is another issue than the one we are scanning for.
            $hostName = hostname
            ...
            Write-EventLog –LogName Application –Source $CustomEventLogSource –EntryType Warning –EventID 669 -Category 0 –Message "Troubleshoot DNS issue yourself"
        }
    }
    Else {

...
$DHCPServer = "nwservices01.image.analysis" #FQDN of the DHCP server
...
Set-DhcpServerv4DnsSetting -ComputerName "nwservices01.image.analysis" -scope 192.168.3.0 -DynamicUpdates "Always" -DeleteDnsRRonLeaseExpiry:$True -UpdateDnsRRForOlderClients:$True

#Enable the Allow and Deny filter for the DHCP server - This is a server-wide setting per IP type, not per scope
Set-DhcpServerv4FilterList -ComputerName "nwservices01.image.analysis" -Allow $True -Deny $True
...
# Create the Deny List for the DHCP filters incode as a table - these are dummy entries as example as we do not have any yet.
...
write-Host -foregroundcolor green "***  Attempting deny list filter number $Counter  ***"
    ...
    If ([string]::IsNullOrEmpty($checkIfMacAddressExistsInDenyFilter)){
        Write-Host -foregroundcolor yellow "MAC Address $($FilterEntry.MacAddress) is not yet used in an existing deny filter entry. Proceeding with check of description."
        If ($FilterEntry.Description -in $CompleteDHCPDenyFilter.description ){
            Write-Host -foregroundcolor yellow "Description $($FilterEntry.Description) has already been used another deny filter entry. Please choose another description to avoid confusion."}
        else{
        ...
        write-Host -ForegroundColor Magenta "deny Filter for MAC Address $($checkIfMacAddressExistsinDenyFilter.MacAddress) with description $($checkIfMacAddressExistsinDenyFilter.Description) allready exists"
...
#Base the DHCP reservation list on the Allow List and extend the tabel with needed colummns.
#
